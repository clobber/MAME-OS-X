			 ...
			 message:(NSString*)message_
{
	if (tryDO) {
		tryDO = NO;
		destination = (id<JRLogDestinationDO>)[[NSConnection rootProxyForConnectionWithRegisteredName:@"JRLogDestinationDO" host:nil] retain];
	}
	if (destination) {
		NS_DURING
...
static JRLogLevel parseJRLogLevel(NSString *level_) {
	...
	return result ? (JRLogLevel)[result intValue] : JRLogLevel_UNSET;
}
